...
@implementation SystemColorType (Colorkit)
...
+(instancetype)colorWithHexString:(NSString *)hexString alpha:(CGFloat)alpha
{
    ...
    return [SystemColorType colorWithRed:redColor / 255.0f green:greenColor / 255.0f blue:blueColor / 255.0f alpha:alpha];
}
...
+ (SystemColorType*)contrastLumaColor:(SystemColorType*)color threshold:(CGFloat)threshold{
	...
	if (relativeLuminance >= threshold) {
		return [SystemColorType whiteColor];
	} else {
		return [SystemColorType blackColor];
	}
}

+ (SystemColorType *)colorWithLuminance:(CGFloat)luminance withA:(CGFloat)a withB:(CGFloat)b{
	CGFloat saturation = sqrt(a * a + b * b);
	CGFloat hue = atan(b/a);
	return [SystemColorType colorWithHue:hue saturation:saturation brightness:luminance alpha:1];
}

+ (instancetype)blendingColorsWithAlphaCompositing:(SystemColorType*)firstColor withColor:(SystemColorType*)secondColor{
	CGFloat r1, g1, b1, a1;
	CGFloat r2, g2, b2, a2;
	...
	return [SystemColorType colorWithRed:blendedRedComponent green:blendedGreenComponent blue:blendedBlueComponent alpha:blendedAlpha];
}
